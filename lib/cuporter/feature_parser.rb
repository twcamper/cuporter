...
module Cuporter
  class FeatureParser
    ...
    def self.parse_names(file)
      self.new(file).parse_names
    end
    ...
    def parse_names
      ...
      lines.each_with_index do |line, i|
        ...
        when SCENARIO_LINE
          # How do we know when we have read all the lines from a "Scenario Outline:"?
          # One way is when we encounter a "Scenario:"
          close_scenario_outline_for_names

          @feature.add_child(Node.new($1))
          @current_tags = []
        when SCENARIO_OUTLINE_LINE
          # ... another is when we hit a subsequent "Scenario Outline:"
          close_scenario_outline_for_names

          @scenario_outline  = Node.new($1)
          @current_tags = []
        when EXAMPLE_SET_LINE, SCENARIO_SET_LINE
    ...
    def close_scenario_outline_for_names
      if @scenario_outline
        if @example_set
          @scenario_outline.add_child(@example_set) if @example_set
          @example_set = nil
        end
        @feature.add_child(@scenario_outline)
        @scenario_outline = nil
      end
    end
