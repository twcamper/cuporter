...
module Cuporter
  class TagNodesParser < FeatureParser
    ...
    def handle_scenario_line(sub_expression)
      s = @feature.filter_child(Node.new_node(:Scenario, @doc, :value => sub_expression, :tags => @current_tags, :number => true))
    ...
    def new_example_line(sub_expression)
      e = Node.new_node(:Example, @doc, :value => sub_expression, :number => true)
      (@current_tags | @example_set.tags | @scenario_outline.tags | @feature.tags).each do |tag|
        next unless @filter.pass? tag.to_a
        @report.add_leaf(e.dup, [:tag, tag], @feature, @scenario_outline, @example_set)
      end
    end
